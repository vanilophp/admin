<section class="row">
    <div class="my-3 col col-xl-4">
        ...
        <div class="input-group">
            ...
            <x-appshell::button type="button" variant="outline-secondary" x-on:click="lookupName()">{{ __('Find by name') }}</x-appshell::button>
        </div>
    </div>
    ...
    <div class="my-3 col col-xl-8" x-show="'empty' === status">
        <div class="alert alert-info">
            {{ __('No products were found by the given name') }}
        </div>
    </div>
        ...
        document.addEventListener('alpine:init', function() {
            Alpine.data('vaniloNameLookup', () => ({
                ...
                lookupName() {
                    ...
                    this.errorMessage = ''
                    let url = '{{ route('vanilo.admin.product.index') }}?name=' + this.searchTerm
                    axios.get(url)
                        .then((response) => {
